...
con <- dbConnect(MySQL(), user=hts_pass$db['user'], password=hts_pass$db['pass'],dbname="projects", host="illumina")
...
for (j in (1:len_a)){
	...
	line <- cat(paste(label,i,j,"",index_final,"","N","","nkatiyar",project_id,sep=","),file="SampleSheet.csv","\n", append=TRUE)
		}
