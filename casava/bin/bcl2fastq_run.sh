...
if [[ ! "${base_mask}" == "NA" ]]; then
    ...
    bcl2fastq --use-bases-mask=$base_mask --runfolder-dir=$run_dir --sample-sheet=$sample_sheet --barcode-mismatches=$barcode_mismatch --minimum-trimmed-read-length 0 --mask-short-adapter-reads 0 --no-lanesplitting --ignore-missing-bcls --ignore-missing-filter --ignore-missing-positions --ignore-missing-controls --processing-threads=64 --demultiplexing-threads=12 --loading-threads=4 --writing-threads=4 --output-dir=$SHARED_GENOMICS/$fc_id/$run_dir 2>&1 | tee $SHARED_GENOMICS/$fc_id/nohup.out
else
    echo -e "\tRunning with out BaseMask"
    bcl2fastq --runfolder-dir=$run_dir --sample-sheet=$sample_sheet --barcode-mismatches=$barcode_mismatch --ignore-missing-bcls --ignore-missing-filter --ignore-missing-positions --no-lanesplitting --ignore-missing-controls --processing-threads=64 --demultiplexing-threads=12 --loading-threads=4 --writing-threads=4 --output-dir=$SHARED_GENOMICS/$fc_id/$run_dir 2>&1 | tee $SHARED_GENOMICS/$fc_id/nohup.out
fi
